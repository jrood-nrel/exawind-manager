...
if [[ "$(spack manager find-machine | awk '{print $2}')" == "ellis" ]]; then
  ...
  cmd "rm -rf /mnt/vdb/home/jrood/combustion/exawind-manager/stage/*"
  cmd "rm -f /mnt/vdb/home/jrood/combustion/exawind-manager/stage/.lock"
fi
...
cmd "time ${EXAWIND_MANAGER}/scripts/deploy.py --daily --cdash ${packages_to_test} --overwrite --regression_tests ${packages_to_test}"
